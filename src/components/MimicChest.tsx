...
export default function MimicChest({ onBonusGold }: MimicChestProps) {
  ...
  return (
    <div className="rpg-panel border border-amber-900/60 bg-[#121822] p-4 text-center">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs font-bold text-amber-300">Suspicious Antique Chest</span>
        <span className="text-[10px] text-slate-400">Curiosity Hazard</span>
      </div>

      <div
        onClick={handleClick}
        className={`cursor-pointer select-none py-3 text-4xl transition-transform ${
